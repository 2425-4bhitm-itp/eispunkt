<script lang="ts">
	...
	onMount(async () => {
		if ($user.role == 'TournamentAdmin') {
			...
			let tournamentsResponse = await fetch(
				`https://it200230.cloud.htl-leonding.ac.at/api/teams/tournament/${$user.team.teamId}`
			);
	...
	function navigateToTournament(tournamentId: number) {
		selectedTournament.set({ selectedTournament: tournamentId });

		if ($user.role == 'TournamentAdmin') {
			goto(`/tournament/selectTeam`);
		} else {
			goto(`/tournament/gameOverview`);
		}
	}
	...
	async function deleteTournament(tournamentId: Number) {
		let deleteResponse = await fetch(`https://it200230.cloud.htl-leonding.ac.at/api/tournaments/${tournamentId}`, {
			method: 'DELETE'
		});

		if (deleteResponse.ok) {
			tournaments = tournaments.filter((t) => t.tournamentId !== tournamentId);
		}
	}
...
<div id="body-div">
	<div id="header-box">
		<a id="backArrow" href="/team" title="Zurück zu den Teams">
		...
		<header id="header">Tournament</header>
	...
	<div id="icon-box">
		<a id="team-icon-box" href="/team" title="Zu den Teams">
			<svg
				width="140"
				height="140"
				viewBox="0 0 40 49"
		...
		<a id="tournament-icon-box" title="Zu den Turnieren">
			<svg
				width="140"
				height="140"
				viewBox="0 0 24 24"
	...
	<div id="tournament-details-outer-box">
		{#if tournaments.length === 0}
			<h3>Keine angemeldeten Turniere vorhanden.</h3>
		{:else}
			{#each tournaments as tournament}
				<div class="tournament-details">
					...
					<div class="svg-box">
						<a
							onclick={() => {
								navigateToTournament(tournament.tournamentId);
							}}
						>
							<svg
								width="40"
